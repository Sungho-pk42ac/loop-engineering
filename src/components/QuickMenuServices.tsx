...
// 스페셜 그리드 아래 서비스 바로가기 줄(#15). 원본: 높이 36 흰 버튼·1px 테두리·radius 4, 24px 아이콘 + 14px/400 라벨,
// md 이상: 가운데 정렬 wrap(좁으면 줄바꿈), 호버 변화 없음. '서비스 전체보기'도 같은 모양.
// md 미만(#16, 원본 모바일): 줄바꿈 없는 한 줄을 이 줄만 가로 스크롤, 앞뒤 16px, 스크롤바 숨김.
// 실측(이슈 255): 라벨 14/400, 아이콘은 24 칸을 그대로 채운다(원본 컬러 이미지 대신 우리 라인 SVG 유지).
      ...
      <ul className="flex w-max gap-1 px-4 md:w-auto md:flex-wrap md:justify-center md:px-0">
        {serviceButtons.map(({ label, icon }) => (
          <li key={label} className="shrink-0">
            <Link
              href="/products"
              className="flex h-9 items-center justify-center gap-1 rounded-sm border border-line bg-surface py-1 pr-2 pl-1 text-body font-regular whitespace-nowrap text-ink"
            >
