...
// 자리표시자 결과: 상품 6종을 반복한다(원본 상품명·사진·가격을 옮기지 않음).
const results = Array.from({ length: GRID_TOTAL }, (_, i) => ({ key: i, product: products[i % products.length] }));
  ...
  return (
    <div className="relative bg-surface-subtle">
      ...
      <ul className={`mx-auto grid max-w-wide md:grid-cols-6 ${mobileCols === 3 ? "grid-cols-3" : "grid-cols-2"}`}>
        {results.slice(0, visible).map(({ key, product }) => (
          <li key={key}>
            <ProductCard product={product} variant="flat" sizes="(min-width: 768px) 17vw, 50vw" />
          </li>
        ))}
      </ul>
