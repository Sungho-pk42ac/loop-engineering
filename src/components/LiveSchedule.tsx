...
// 원본은 페이지 마지막 섹션(푸터 바로 위). 좌우 여백은 원본 실측대로 모든 폭 16px(#173), 스냅은 scroll-px 로 여백을 지킨다.
  ...
  return (
    <section aria-labelledby="live-schedule-title" className="pb-4">
      <div className="flex h-12 items-center justify-between px-4">
        ...
        <Link href="/products" {...newTab} className="-mr-1 px-1 py-2 text-label text-ink-muted underline">
          더보기
        </Link>
      </div>
      <ScrollRow
        listClassName="scrollbar-none flex gap-1 overflow-x-auto px-4 md:snap-x md:snap-mandatory md:scroll-px-4"
        prevLabel="이전 라이브 보기"
        nextLabel="다음 라이브 보기"
      >
          ...
          <li key={live.id} className="w-34 shrink-0 snap-start md:w-65">
            <Link href="/products" {...newTab} className="flex h-full flex-col overflow-hidden rounded-sm bg-surface">
              ...
              <div className="px-2 pt-2 pb-3">
                ...
                <p className="mt-1 line-clamp-4 text-label text-ink-muted">{live.benefit}</p>
              </div>
            </Link>
          </li>
        ))}
