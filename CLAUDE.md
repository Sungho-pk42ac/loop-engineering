...
강의 실습용 간단 쇼핑몰. 상품 목록과 상품 상세 조회만 제공하는 **프론트엔드 전용** Next.js 앱이다. 백엔드·DB·API 라우트·장바구니·결제는 **의도적으로 범위에서 제외**되어 있으므로 새로 추가하지 말 것. 인증은 localStorage 목업(회원가입/로그인/로그아웃, 서버 없음)만 포함한다. 모든 상품 데이터는 하드코딩된 정적 배열에서 직접 import 한다.
