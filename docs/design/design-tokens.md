...
| | `surface-floating` | white 90% (다크: black 90%) | 콘텐츠 위에 떠 있는 반투명 알약·바(성별 토글 #222). 뒤가 무엇이든 글자 대비가 보장되는 알파 |
...
화면에 고정된 요소의 `bottom-*`·`top-*` 은 위 8종이 아니라 **가려지는 고정 요소의 높이에서 파생**한다. 예: 성별 토글(#222)은 모바일에서 하단 탭바(#139 `h-14` = 56) 위 12px 이라 `bottom-17`(68), 탭바가 없는 md 이상은 원본대로 `bottom-3`(12). 탭바 높이가 바뀌면 이 값도 같이 바꾼다.
